...
-- name: GetPropertiesForGeoJson :many
WITH facilities_and_amenities AS(
    SELECT p.id,fa."type",
    array_agg(DISTINCT coalesce(fae.facility_amenity_id,0))::bigint[] AS facilities_amenities
    FROM property p
    LEFT JOIN facilities_amenities_entity fae ON fae.entity_id=p.id AND fae.entity_type_id= @property_type::bigint
...
property_media_agg AS (
    ...
    FROM 
        global_media
    where global_media.entity_type_id = @property_type::bigint
    or  global_media.entity_type_id = @property_version_type::bigint
    GROUP BY 
...
MinDates AS (
    ...
    JOIN 
        ...
        AND payment_plans_packages.entity_type_id = @property_type::bigint
    JOIN 
        plan_installments 
...
DesiredCountries as (
    ...
    JOIN swap_requirement ON swap_requirement.entity_id=property_versions.id AND swap_requirement.entity_type= @property_version_type::bigint
...
media as(
	...
	JOIN global_media 
	  ON   ( case when property_versions.has_gallery is null OR property_versions.has_gallery=FALSE then property.id = global_media.entity_id and global_media.entity_type_id= @property_type::bigint
    else global_media.entity_id=property_versions.id and global_media.entity_type_id= @property_version_type::BIGINT END) 
    where 
...
LEFT JOIN phases ON phases.id= p.entity_id AND p.entity_type_id= @phase_type::bigint 
JOIN property_media_agg ON ( case when pv.has_gallery is null OR pv.has_gallery=FALSE then p.id = property_media_agg.entity_id 
    else property_media_agg.entity_id=pv.id and property_media_agg.entity_type_id= @property_version_type::BIGINT END)
...
LEFT JOIN DesiredCountries 
    ...
    AND pv.category= @swap_category::BIGINT
WHERE cu.company_id= @company_id::BIGINT and pv.status not in (1,5,6) and users.status=2 AND gpt."usage"= any(@usage::BIGINT[]);



-- name: GetUnitsForGeoJson :many
WITH facilities_and_amenities AS(
    SELECT uv.id,
    fa."type",
    array_agg(DISTINCT coalesce(fae.facility_amenity_id,0))::bigint[] AS facilities_amenities
    ...
    LEFT JOIN facilities_amenities_entity fae ON fae.entity_id=u.id AND fae.entity_type_id= @unit_entity_type::bigint 
    LEFT JOIN facilities_amenities fa ON fae.facility_amenity_id=fa.id 
...
media as(
	...
	JOIN global_media 
	  ON   ( case when unit_versions.has_gallery is null OR unit_versions.has_gallery=FALSE then units.id = global_media.entity_id  and global_media.entity_type_id= @unit_entity_type::bigint 
    else global_media.entity_id=unit_versions.id and global_media.entity_type_id= @unitversions_entity_type::BIGINT END)
    where 
	     global_media.media_type = 1 
	    AND global_media.gallery_type = 'Main'
)
